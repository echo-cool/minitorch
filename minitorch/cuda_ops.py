...
def tensor_map(
    ...
    def _map(
        ...
        if 0 <= i <= out_size:
            ...
            sj = index_to_position(in_index, in_strides)
            temp = fn(in_storage[sj])
            out[out_i] = temp
...
def tensor_zip(
    ...
    def _zip(
        ...
        if 0 <= i < out_size:
            out_i = index_to_position(out_index, out_strides)
            sj = index_to_position(a_index, a_strides)
            sk = index_to_position(b_index, b_strides)
            temp = fn(a_storage[sj], b_storage[sk])
            out[out_i] = temp
...
def tensor_reduce(
    ...
    def _reduce(
        ...
        if local_i < a_red:
            ...
            cuda.syncthreads()  # sync

        if local_i < a_red and pos == 0:
            acc = reduce_value  # gives our local variable the reduce value
